...
This sensor uses a stretchable band that must be placed around the user’s ribs and will detect when the user is inhaling and exhaling.
...
The Bitalino card (see Figure 4) is responsible for gathering all the data coming from the three sensors and sending them to the OpenSignals software. The card possesses multiple ports beginning with “A”, being the ones which the sensors need to be connected to.\
Make sure to connect the ECG Sensor to “A1”, the EDA Sensor to “A2” and the Breathing Sensor to “A3”. Then, turn on the card so it can connect by Bluetooth connection to the OpenSignals software.
...
###    c. Sending the datat to the Python program

The sensors data can be sent to a python program using Lab Streaming Layer. To do so, you need to activate the “Lab Streaming Layer” parameter in the “Integration” section of the settings (the button next to the one used for connecting to the Bitalino card).
...
The Python program is responsible for converting the raw data into MIDI notes. To do so, the program receives the values from OpenSignals and detects when a pulse is happening or if the user is inhaling or exhaling. During these events the program sends different MIDI notes to multiple programs to impact the music and the visuals displayed. This program is called "ArtOfLife.py" and can be found on this GitHub repository.
...
For the Python program to be able to send MIDI notes, the computer needs to open virtual MIDI ports, and so is the role of loopMIDI. loopMIDI is a very simplistic software that can open personalized MIDI ports. You will need to create two ports that will link to the FL Studio software. Make sure to call them “loopMIDI Port FL” and “loopMIDI Port FL2”. Then, you will need to connect two ports that will connect to the MadMapper software. Make sure to call them “loopMIDI Port MM” and “loopMIDI Port MM2”.
...
FL Studio 20 is the music software used in this project. The software will play a predefined looped sample of music while display sounds reacting to the MIDI notes it receives from loopMIDI. The FL Studio file is called “ArtOfLife.flp” and can be found on this GitHub repository.
